...
/**
 * KA Global IP – Patent Renewal Fees by Country: Austria
 * -------------------------------------------------------
 * Path:
 * /app/solutions/renewal-fees-by-country/austria/page.tsx
 *
 * Brand colors:
 *  - Primary: #7F2B38
 *  - Accent:  #4D1475
...
const eur = (n: number) =>
  new Intl.NumberFormat("en-AT", { style: "currency", currency: "EUR" }).format(n);

// ---------- Sample placeholder fee data ----------
const SAMPLE_FEES = Array.from({ length: 20 }, (_, i) => {
  const year = i + 1;
  // Austria: renewals are due annually from Year 3 onward
  const official = year < 3 ? 0 : 100 + year * 20; // placeholder growth
  const service = official > 0 ? 65 : 0;
  const lateSurcharge = official > 0 ? Math.round(official * 0.2) : 0; // illustrative
        ...
        <div className="grid items-center gap-8 md:grid-cols-12">
          <div className="md:col-span-7">
            <h1 className="text-3xl md:text-5xl font-extrabold leading-tight">
              Austria — Patent Renewal Fees (2025 Guide)
            </h1>
            <p className="mt-4 text-base md:text-lg text-white/90">
              Keep your Austrian patents in good standing with transparent fees, reminders, and global
              coverage. This page outlines typical renewal schedules, cost components, and our managed
              service options for Austria.
            </p>
    ...
    <section className="bg-[#F0F0F0] py-8 md:py-10">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <FactCard label="Office" value="Austrian Patent Office (ÖPA)" icon={<Info className="h-5 w-5" />} />
  ...
  return (
    <section id="fees" className="mx-auto max-w-7xl px-6 py-12">
      ...
      <p className="mt-3 text-xs text-gray-600">
        * Our team will confirm applicable grace periods and surcharges with the Austrian Patent Office before payment.
      </p>
    </section>
  );
}
...
function HowItWorks() {
  const steps = [
    {
      title: "Share details",
      text: "Provide AT or EP application/grant number and owner details. We run a docketing check and verify due dates.",
    },
    {
  ...
  return (
    <section className="bg-[#F0F0F0] py-12">
      <div className="mx-auto max-w-7xl px-6">
        <h2 className="text-2xl md:text-3xl font-bold text-[#4D1475]">How KA Global IP handles AT renewals</h2>
      ...
      <div className="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-6 md:p-8 grid gap-6 md:grid-cols-12">
        <div className="md:col-span-9">
          ...
          <p className="mt-2 text-gray-700 text-sm md:text-base">
            Renewals affect portfolio value. We map AT renewals to your global strategy—pruning low-value assets, extending core
    ...
    <section className="mx-auto max-w-7xl px-6 py-10">
      <div className="grid gap-6 md:grid-cols-12">
        <div className="md:col-span-8">
          <h2 className="text-2xl md:text-3xl font-bold text-[#4D1475]">Deadlines & practical notes (AT)</h2>
          <div className="mt-4 space-y-3 text-sm text-gray-800">
            <p>• Austrian renewals are due annually from the <strong>6th year</strong> onward.</p>
            ...
            <p>• For EP validations, annuities shift to the Austrian Patent Office post-grant.</p>
            <p>• Keep ownership and address of service current to avoid missed notices. We can assist with recordals before the next renewal.</p>
            <p>• For portfolios, consider bundling AT renewals with other jurisdictions for consolidated billing and discounted service fees.</p>
          </div>
        </div>
        <div className="md:col-span-4">
          <div className="rounded-2xl border border-gray-200 bg-[#F0F0F0] p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-gray-800">
              <li>• AT or EP application or patent number</li>
...
function CountryNav() {
  const countries = [
    ...
    { name: "Australia", slug: "australia" }, // show AU here, exclude current (Austria)
...
function FAQ() {
  const faqs = [
    {
      q: "When do renewal fees start for Austrian patents?",
      a: "From the 6th year, annually until expiry. We confirm your exact schedule with the Austrian Patent Office.",
    },
    {
      q: "Is there a grace period if we miss a due date?",
      a: "Typically up to 6 months with surcharge, depending on timing and case specifics. We’ll advise on the best course immediately.",
    },
    {
      q: "Do you manage EP validations in Austria?",
      a: "Yes. Once the EP is granted and validated in Austria, subsequent renewals are payable to the Austrian Patent Office.",
    },
    {
      q: "Do you assist with recordals before renewal?",
      a: "Absolutely. We can update ownership or address of service to reduce risk of missed notices.",
    },
  ];
  return (
    <section className="bg-[#F0F0F0] py-12">
    ...
    <section id="quote" className="relative isolate">
      ...
      <div className="mx-auto max-w-7xl px-6 py-14 text-white">
        <div className="grid items-center gap-6 md:grid-cols-12">
          <div className="md:col-span-8">
            <h2 className="text-2xl md:text-3xl font-extrabold">Need a confirmed AT quote today?</h2>
            <p className="mt-2 text-white/90">
              Send the AT application/grant number and desired action. We’ll verify deadlines, confirm official fees, and reply with a final quote.
            </p>
          </div>
          <div className="md:col-span-4 md:text-right">
            <a
              href="mailto:support@kaglobalip.com?subject=Austria%20Renewal%20Quote"
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-8 text-xs text-gray-600">
      <p>
        ...
        fees and rules change. KA Global IP will verify all details with the Austrian Patent Office before filing.
...
function StickyHelpCard() {
  return (
    <div className="sticky top-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-2 text-sm text-gray-700">Share your AT patent number and we’ll confirm the actual due date and fees.</p>
      <a
        href="mailto:support@kaglobalip.com?subject=Help%20with%20AT%20Renewal"
...
export default function AustriaRenewalFeesPage() {
  return (
    <>
      <Head>
        <title>Austria Patent Renewal Fees (2025 Guide) | KA Global IP</title>
        <meta
          name="description"
          content="Indicative renewal fees, timelines and managed services for Austrian patents. Attorney-led, ISO-certified renewals across 100+ jurisdictions."
        />
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "WebPage",
              name: "Austria Patent Renewal Fees (2025 Guide)",
              description: "Indicative renewal fees, deadlines and managed services for AT patents.",
