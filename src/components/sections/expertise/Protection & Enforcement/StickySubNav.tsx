...
export default function StickySubNav() {
    ...
    return (
        <div className="sticky top-0 z-30 border-b border-gray-200 bg-white/80 backdrop-blur">
            <motion.div className="h-0.5 bg-gradient-to-r from-[#7F2B38] via-[#6E44AB] to-[#4B2E83]" style={{ scaleX: progress, transformOrigin: "0 0" }} />
                ...
                <nav className="flex gap-2 overflow-x-auto py-3">
                    {TABS.map((t) => (
                        <a
                            key={t.id}
                            href={`#${t.id}`}
                            className={`whitespace-nowrap rounded-full px-3 py-1.5 text-sm ${active === t.id ? "bg-[#efeafd] text-[#4B2E83] ring-1 ring-[#7b61c3]/20" : "text-gray-600 hover:text-gray-900"
                                }`}
                        >
                            {t.label}
                        </a>
