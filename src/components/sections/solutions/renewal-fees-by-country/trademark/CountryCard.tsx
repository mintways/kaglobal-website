// components/sections/renewals/TrademarkCountryCard.tsx
"use client";
...
function StatusPill({ status }: { status?: CountryInfo["status"] }) {
    ...
    return (
        <span className={`rounded-full px-2 py-1 text-[11px] font-semibold shadow-sm ring-1 ${styleMap}`}>
            {label}
        </span>
    );
}

export default function TrademarkCountryCard({ c }: Props) {
    const disabled = c.status === "coming-soon";

    return (
        <Link
            href={disabled ? "#" : `/solutions/renewal-fees-by-country/trademark/${c.slug}`}
            aria-label={`View ${c.name} trademark renewal fees`}
        ...
        >
            ...
            <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-gray-600 line-clamp-2">
                    {c.subtitle ?? "Trademark renewal (maintenance) overview"}
                </p>
            </div>
            ...
            <div className="mt-5 flex items-center justify-between text-sm text-gray-500">
                <span className="inline-flex items-center gap-2">
                    ...
                    {c.status === "updated" ? "Recently updated" : "Read More"}
                </span>
