...
const Icon = ({ name }: { name: string }) => {
    ...
    switch (name) {
        ...
        case "sparkle":
            return (
                <svg viewBox="0 0 24 24" className={cls} fill="currentColor" aria-hidden>
                    <path d="M12 2 9 9l-7 3 7 3 3 7 3-7 7-3-7-3-3-7Zm7 2-1 3-3 1 3 1 1 3 1-3 3-1-3-1-1-3Z" />
                </svg>
            );
        case "handshake":
            return (
                <svg viewBox="0 0 24 24" className={cls} fill="currentColor" aria-hidden>
                    <path d="M9.5 7 12 9.5l2.5-2.5a3.5 3.5 0 0 1 5 5l-4.7 4.7a2.5 2.5 0 0 1-3.6 0l-.2-.2-.9.9a2.5 2.5 0 0 1-3.5-3.5l.9-.9-.2-.2a2.5 2.5 0 0 1 0-3.6L9.5 7Z" />
                </svg>
            );
        default:
            return (
                <svg viewBox="0 0 24 24" className={cls} fill="currentColor" aria-hidden>
                    <path d="M7 10V8a5 5 0 1 1 10 0v2h1a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h1Zm2 0h6V8a3 3 0 0 0-6 0v2Z" />
                </svg>
            );
    }
};
            ...
            <div className="relative mx-auto max-w-7xl px-6 py-16 md:py-20">
                ...
                <div className="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-4">
                    {VALUES.map((v) => (
                        <div
                            key={v.t}
                            className="group relative overflow-hidden rounded-3xl border border-black/10 bg-white/80 p-6 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:shadow-md font-ptsans"
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <h3 className="font-playfair text-base font-semibold text-gray-900">{v.t}</h3>
                                <p className="mt-2 text-sm text-gray-700 font-ptsans">{v.d}</p>

                                {/* chips row */}
                                <div className="mt-4 flex flex-wrap gap-2 font-ptsans">
                                    {v.t === "Integrity" &&
                                        ["Transparency", "Accountability"].map((c) => (
                                            <span key={c} className="rounded-full border border-black/10 bg-white px-3 py-1 text-xs text-gray-700 shadow-sm">
                                                {c}
                                            </span>
                                        ))}
                                    {v.t === "Excellence" &&
                                    ...
                                    {v.t === "Client-First" &&
                                        ["Outcome-Driven", "Speed"].map((c) => (
                                            <span key={c} className="rounded-full border border-black/10 bg-white px-3 py-1 text-xs text-gray-700 shadow-sm">
                                                {c}
                                            </span>
                                        ))}
                                    {v.t === "Security" &&
                                        ["Privacy", "Clean Rooms"].map((c) => (
                                            <span key={c} className="rounded-full border border-black/10 bg-white px-3 py-1 text-xs text-gray-700 shadow-sm">
                                                {c}
                                            </span>
                                        ))}
